...
import {
  headlineImpactMetrics,
  programmeInvestmentShares,
      ...
      <div className='site-container'>
        <div className='grid gap-8 border-b border-slate-300 pb-10 lg:grid-cols-[0.8fr_1.2fr] lg:items-end'>
          ...
          <div className='max-w-2xl lg:justify-self-end'>
            <p className='text-lg leading-8 text-slate-600'>
              These cumulative organisational figures show the scale of CeRID’s
              reach, investment, and participation by women and girls.
            </p>
            <p className='mt-3 flex items-start gap-2 text-sm leading-6 text-slate-500'>
              ...
              Figures are drawn from the organisational overview supplied by CeRID.
            </p>
          </div>
        </div>
        ...
        <div className='mt-16'>
          <div className='max-w-3xl'>
            <p className='eyebrow'>How investment is distributed</p>
            <h2 className='mt-4 text-3xl font-extrabold tracking-[-0.03em] text-slate-950 md:text-4xl'>
              Programme investment by area.
            </h2>
            <p className='mt-5 text-lg leading-8 text-slate-600'>
              The shares below account for CeRID’s full programme investment portfolio.
            </p>
          </div>
          <dl className='mt-10 grid border-y border-slate-300 sm:grid-cols-2 lg:grid-cols-3'>
            {programmeInvestmentShares.map((area, index) => (
              <div
                key={area.label}
                className={`border-slate-300 py-8 sm:px-7 ${
                  index > 0 ? 'border-t sm:border-t-0' : ''
                } ${index % 2 === 1 ? 'sm:border-l' : ''} ${
                  index > 1 ? 'lg:border-l' : ''
                }`}
              >
                <dd className='text-4xl font-extrabold tracking-[-0.04em] text-brand'>
                  {area.value}
                </dd>
                <dt className='mt-3 max-w-sm font-bold leading-7 text-slate-950'>
                  {area.label}
                </dt>
              </div>
            ))}
